...
export default function QuizzPage() {
    ...
    useEffect(() => {
        setQuestions(shuffle(quizz.questions).map((item, index) => {
            return (
    ...
    const submit = () => {
        ...
        if (checkboxes.length < 10 && !endTimer) {
            ...
            message.innerHTML = "Toutes les questions n'ont pas de réponse et il vous reste encore du temps"
            return
        } else {
        ...
        if(checkboxes.length > 0){
            for (let i = 0; i < checkboxes.length; i++) {
                ...
                const userresponse = quizz.questions.find((item) => item.id == j).reponses.find(
                    (item) => item.texte == checkboxes[i].value,
                )
        ...
        allInputs.forEach((input) => {
            ...
            if(quizz.questions.find((question) => question.id == idQ).reponses.find((reponse) => reponse.texte == input.value).vrai){
    ...
    return (
        <div className='card mt-5 quizz-card'>
            ...
            <div className='card-header text-center'>
                <Link className='btn btn-dark' to={`../../quizz`} ><FontAwesomeIcon icon={faArrowLeftLong} style={{color: "#ffffff",}} /></Link>
            ...
            <div className='card-body'>
                ...
                { showRestartButton ? <button className='btn btn btn-dark ms-3' onClick={() => window.location.reload()} >Recommencer</button> : null }
            </div>
        </div>
    )
}
